...
function generateMockDiagnosis(proposal: string): any[] {
  ...
  if (!hasRQ) {
    items.push({
      ...
      suggestion: '明确写出你的 Research Question，确保它"不 obvious 但不止一个可能答案"（Interestingness 恰当档位）。',
      severity: 'error',
    ...
    items.push({
      category: '研究问题质量',
      point: '研究问题已初步提出',
      reason: '你已经有明确的研究方向，这是好的开始。但需要进一步判断它落在 Interestingness 的哪一档。',
      suggestion: '对照"走得太远"（套路化新颖、过度反直觉）和"不够"（直白显然）两档，判断你的 RQ 是否处于恰当位置。',
      severity: 'ok',
  ...
  if (!hasWhatWeKnow) {
    items.push({
      ...
      suggestion: '系统梳理与你的 RQ 相关的已有文献理解，不只是定义，而是关键发现和理论视角。',
      severity: 'error',
    ...
    items.push({
      ...
      suggestion: '尝试用"三步归纳法"：宽分类 → 细分类 → 整合框架，来组织你的文献综述。',
      severity: 'warning',
  ...
  if (!hasWhatWeDontKnow) {
    items.push({
      ...
      reason: '仅仅描述已有发现不够，必须指出现有文献的不足或矛盾，才能定位你的贡献。',
  ...
  if (!hasWhatWeDontKnow) {
    ...
    items.push({
      category: 'SO WHAT 诊断',
      point: 'SO WHAT 陈述不够清晰——读者无法判断你的研究为什么重要',
      reason: '即使指出了研究空白，如果没有说明这个空白为什么值得填补，proposal 仍然缺乏说服力。',
      suggestion: '用一句话清楚陈述你的研究的理论贡献：它将 change / challenge / fundamentally advance 哪个已有理解。避免"增加一个新情境下的实证例子"这种不够贡献性的表述。',
      severity: 'warning',
      knowledgePoint: ref(['contribution', '理论贡献']),
    });
  } else {
    items.push({
      ...
      reason: 'SO WHAT 不能只是一个模糊的"填补空白"。需要明确你的研究如何具体地改变、挑战或推进已有理解。',
      suggestion: '将 SO WHAT 精确定位到具体文献的具体结论或假设——你的研究将对哪个已知关系、哪个边界条件、哪个理论机制提出新的理解。',
      severity: 'ok',
      knowledgePoint: ref(['contribution', '理论贡献']),
    });
  }
  ...
  if (/假设|命题|hypothesis/.test(proposal) && !hasRQ) {
    items.push({
      category: '逻辑一致性',
      point: '假设似乎先于研究问题提出',
      reason: '研究问题应该导致你的假设，而不是反过来。不要为一个你已有的答案编造问题。',
      suggestion: '从你的研究问题出发，推导出假设。确保假设是 RQ 的自然延伸，而非预先设定的结论。',
      severity: 'warning',
      knowledgePoint: ref(['contribution', '理论贡献']),
    });
  }
  ...
  items.push({
    category: '贡献定位',
    point: '需要明确你的研究属于 change / challenge / fundamentally advance 哪个层面',
    reason: '仅仅增加一个新的实证例子不是理论贡献。需要改变、挑战或根本推进我们对该领域的理解。',
    suggestion: '明确陈述你的研究将如何改变、挑战或推进已有理解，而不是只增加一个"新情境下的验证"。',
    severity: 'warning',
    knowledgePoint: ref(['contribution', '理论贡献']),
  });
